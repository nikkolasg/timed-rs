...
use timed_core::{get_output, set_output, Output, TIMED_OUTPUT_ENV, refresh_from_env};
...
fn cleanup_csv_file(filename: &str) {
    if Path::new(filename).exists() {
        fs::remove_file(filename).expect(&format!("Failed to remove existing CSV file: {}", filename));
    }
    assert!(!Path::new(filename).exists(), "CSV file still exists after cleanup: {}", filename);
}
...
fn verify_csv_file(filename: &str) {
    ...
    assert!(Path::new(filename).exists(), 
            "CSV file wasn't created when using CSV output: {}", filename);
    
    // Read contents of the CSV file
    let csv_content = fs::read_to_string(filename)
        .expect(&format!("Failed to read CSV file: {}", filename));
    
    // Log the content for debugging
    println!("CSV content: {}", csv_content);
    
    // Verify CSV file has the header
    assert!(csv_content.contains("function") && csv_content.contains("duration_ms"), 
            "CSV header doesn't contain expected columns: {}", filename);
    ...
    assert!(csv_content.contains("test_function_default_level") || 
            ...
            "CSV {} doesn't contain any function names: {}", filename,csv_content);
}
